            ...
            for (FileItem item : items) {
                if (!item.isFormField()) {
                    if (!item.getName().isEmpty()) {
                        nombreRecibo = new File(item.getName()).getName();
                        datosRecibo = item.get();
                        tipoRecibo = item.getContentType();
                    }
                }
            }
            
            // Only update if a file was selected
            if (datosRecibo != null && datosRecibo.length > 0) {
                Connection conn = ((javax.sql.DataSource) pageContext.getAttribute("trabajosdegradoBD")).getConnection();
                String updateSql = "UPDATE anteproyecto SET recibo_pago = ?, recibo_nombre = ?, recibo_tipo = ? WHERE id = ?";
                PreparedStatement ps = conn.prepareStatement(updateSql);
                ps.setBytes(1, datosRecibo);
                ps.setString(2, nombreRecibo);
                ps.setString(3, tipoRecibo);
                ps.setInt(4, Integer.parseInt(idAnteproyecto));
                ps.executeUpdate();
                ps.close();
                conn.close();
                actualizado = true;
                nombreArchivoSubido = nombreRecibo;
            }
...
<head>
    ...
    <title>Adjuntar Recibo de Pago</title>
...
<div class="container mt-5">
    <div class="card shadow">
        <div class="card-header">
            <h2 class="text-center m-0">Adjuntar Recibo de Pago</h2>
        </div>
        <div class="card-body p-4">
            <sql:query dataSource="${trabajosdegradoBD}" var="detalle">
                SELECT a.id, a.recibo_nombre, a.id_director, a.id_evaluador,
            ...
            <c:forEach var="ap" items="${detalle.rows}">
                <form method="post" enctype="multipart/form-data">
                    <div class="current-file mb-4">
                        <h5><i class="fas fa-receipt me-2"></i>Recibo de pago actual:</h5>
                        <c:choose>
                            <c:when test="${not empty ap.recibo_nombre}">
                                <a href="verReciboPago.jsp?id=${ap.id}" target="_blank">
                                    <i class="fas fa-download me-1"></i>${ap.recibo_nombre}
                                </a>
                            </c:when>
                            <c:otherwise>
                                <span class="text-muted">No hay recibo de pago subido</span>
                            </c:otherwise>
                        </c:choose>
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-bold">Nuevo recibo de pago (PDF):</label>
                    ...
                    <div class="text-center mt-4">
                        <button type="submit" class="btn btn-warning btn-lg">
                            <i class="fas fa-paperclip me-2"></i>Adjuntar Recibo de Pago
                        </button>
...
<script>
    ...
    <% if (actualizado) { %>
        Swal.fire({
            icon: 'success',
            title: '¡Actualizado!',
            text: 'Se Adjuntó el recibo de pago: <%= nombreArchivoSubido %>',
