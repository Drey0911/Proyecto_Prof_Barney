...
<c:if test="${pageContext.request.method eq 'POST' and not fn:contains(pageContext.request.contentType, 'multipart/form-data')}">
    ...
    <c:if test="${accion eq 'calificar'}">
        ...
        <c:choose>
            <%-- Si el estado de calificación del coordinador está vacío, se actualiza ese campo --%>
            <c:when test="${empty anteRec.rows[0].estado_calificacion_coordinador}">
                <sql:update dataSource="${trabajosdegradoBD}">
                    UPDATE anteproyecto 
                    SET estado_calificacion_coordinador = ?, id_coordinador = ?
                    WHERE id = ?
            ...
            <%-- Si ya se encuentra "Aprobado" el estado en coordinador, y además en director y evaluador, se actualiza la calificación final --%>
            <c:when test="${anteRec.rows[0].estado_calificacion_coordinador eq 'Aprobado'
                             and anteRec.rows[0].estado_calificacion_director eq 'Aprobado'
                             and anteRec.rows[0].estado_calificacion_evaluador eq 'Aprobado'}">
                <sql:update dataSource="${trabajosdegradoBD}">
                    UPDATE anteproyecto 
                    SET calificacion_total = ?
                    WHERE id = ?
                    <sql:param value="${param.calificacion}" />
                    <sql:param value="${param.anteproyectoId}" />
                </sql:update>
            </c:when>
            <%-- En cualquier otro caso (por ejemplo, no cumple la condición de todos Aprobados), se actualiza el estado del coordinador --%>
            <c:otherwise>
                <sql:update dataSource="${trabajosdegradoBD}">
                    UPDATE anteproyecto 
                    SET estado_calificacion_coordinador = ?
                    WHERE id = ?
                    <sql:param value="${param.calificacion}" />
                    <sql:param value="${param.anteproyectoId}" />
                </sql:update>
        ...
        <c:redirect url="calificacionCoorGestion.jsp?id=${idCoordinador}&rol=${rol}&nombre_rol=${nombreRol}&nombres=${nombres}" />
    </c:if>
</c:if>
...
<sql:query dataSource="${trabajosdegradoBD}" var="listaAnteproyectos">
    ...
    WHERE a.recibo_pago IS NOT NULL
      ...
      AND (
             a.estado_calificacion_coordinador NOT IN ('Aprobado', 'No Aprobado', 'Con Cambios')
             OR (
                  a.estado_calificacion_coordinador = 'Aprobado'
                  AND a.estado_calificacion_director = 'Aprobado'
                  AND a.estado_calificacion_evaluador = 'Aprobado'
                  AND (a.calificacion_total IS NULL OR a.calificacion_total = '')
                 )
          )
                ...
                <tbody>
                    <c:forEach var="ap" items="${listaAnteproyectos.rows}">
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-${empty ap.calificacion_total ? 'secondary' : (ap.calificacion_total eq 'No Aprobado' ? 'danger' : 'success')}">
                                    <c:out value="${empty ap.calificacion_total ? 'Pendiente' : ap.calificacion_total}" />
                                </span>
                            </td>
                            <td>
